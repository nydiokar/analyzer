...
POST /api/v1/analyses/wallets/trigger-analysis
// → Internally creates jobs + polls for completion
// → Returns final result (maintains existing behavior)
// → Gradual migration path for frontend

POST /api/v1/analyses/similarity
// → Creates job + returns immediate status
// → Frontend can choose polling vs WebSocket updates
```
