...
1.  **Implement `GET /wallets/{walletAddress}/runs/latest/aggregated-metrics` API Endpoint:** Develop the endpoint to provide comprehensive metrics from the latest `AdvancedStatsResult` for a wallet. (Corresponds to plan section 4.3.3)
2.  **Implement `GET /wallets/{walletAddress}/runs/latest/pnl-overview` API Endpoint:** Develop the endpoint to provide a top-level PNL overview, potentially drawing from `PnlAnalysisService`. (New, as discussed)
...
5.  **Implement `GET /wallets/{walletAddress}/token-performance` API Endpoint:** Develop the endpoint to provide paginated and sortable token performance data from the latest `AnalysisResult` for a wallet. (Corresponds to plan section 4.3.2)
-->
