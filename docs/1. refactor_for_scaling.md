...
1.  **Implement `GET /wallets/{walletAddress}/runs/latest/token-performance` API Endpoint:** Develop the endpoint to provide paginated and sortable token performance data from the latest `AnalysisResult` for a wallet. (Corresponds to plan section 4.3.2)
2.  **Implement `GET /wallets/{walletAddress}/runs/latest/aggregated-metrics` API Endpoint:** Develop the endpoint to provide comprehensive metrics from the latest `AdvancedStatsResult` for a wallet. (Corresponds to plan section 4.3.3)
3.  **Implement `GET /wallets/{walletAddress}/runs/latest/pnl-overview` API Endpoint:** Develop the endpoint to provide a top-level PNL overview, potentially drawing from `PnlAnalysisService`. (New, as discussed)

<!-- 
Previously listed tasks (now complete):
...
4.  **Refactor Services for API User Context:** Adapt existing analysis services (`BehaviorService`, `PnlAnalysisService`, etc.) to accept a `userId` or user context, enabling them to use `DatabaseService.logActivity()` when invoked via the API. 
    (See note in section 4.4 regarding the refined approach to this task).
-->
...
#### Next Endpoints to Implement (Dashboard Focus):

1.  **`GET /wallets/{walletAddress}/runs/latest/token-performance`**:
...
2.  **`GET /wallets/{walletAddress}/runs/latest/aggregated-metrics`**:
    *   **Plan Reference:** Section 4.3.3.
    *   **Purpose:** Powers the "aggregated metrics" view for the dashboard.
...
3.  **`GET /wallets/{walletAddress}/runs/latest/pnl-overview`** (Tentative name, can be refined):
    *   **Purpose:** Provide a top-level PNL overview for a wallet, potentially drawing from the main summary object produced by `PnlAnalysisService`.
    *   **Data Considerations:** Could include overall realized PNL, total SOL spent/received if not covered adequately elsewhere, and key ratios or summary figures derived by `PnlAnalysisService` that are not part of `AdvancedStatsResult` or the detailed token list.
    *   **Services Used:** `PnlAnalysisService`, `DatabaseService`.
    *   **Note:** This endpoint aims to provide what might be directly used by a PNL summary section in the dashboard, distinct from the granular token list or the statistical aggregates.

#### 4.3.2 Token Performance View Endpoints
-   **`GET /wallets/{walletAddress}/runs/latest/token-performance`**: (Moved to "Next Endpoints to Implement")
...
-   **`GET /wallets/{walletAddress}/runs/latest/aggregated-metrics`**: (Moved to "Next Endpoints to Implement")
