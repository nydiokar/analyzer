...
1.  **Dashboard Project Setup & Core Dependencies Integration:**
    *   Initialize the Next.js (App Router) frontend project.
    *   Integrate essential styling and component libraries: TailwindCSS, shadcn/ui, and Tremor.
    *   Set up the charting library (Apache ECharts), data fetching library (SWR or React Query), and global state management (Zustand or React Context).
    *   (Corresponds to Phase 5.3, Week 1, tasks 1-6)

2.  **Implement Dashboard Layout Skeleton & Basic Routing:**
    *   Define core application routing (e.g., `/wallets/[walletAddress]`, `/settings`).
    *   Build the main `WalletProfileLayout` component, including the Sidebar, Top Sticky Summary Bar area, and the container for tabbed data views.
    *   Create initial component stubs for `AccountSummaryCard` and `TimeRangeSelector`.
    *   (Corresponds to Phase 5.3, Week 1, tasks 7-10)
...
3.  **Develop Global Time Filter & Connect Sticky Summary Bar:**
    ...
    *   Connect the "Sticky Summary Bar" (within `WalletProfileLayout`) to the `GET /wallets/{walletAddress}/summary` API endpoint, populating it with live data.
...
*   **Tasks:**
    1.  Initialize Monorepo (if not already) or Frontend Project: Next.js with App Router.
    2.  Integrate TailwindCSS.
    3.  Configure shadcn/ui: Initialize, add necessary primitive components (buttons, tabs, dropdowns, cards, tables, date pickers).
    4.  Integrate Tremor: Add to project for metric display components.
    5.  Integrate Apache ECharts.
    6.  Setup Data Fetching Library (SWR/React Query) and State Management (Zustand/Context).
    7.  Define Routing: Main pages (`/wallets/[walletAddress]`, `/settings`, `/help`).
    8.  Sidebar Implementation: Use shadcn/ui for structure and navigation items.
    9.  Main Layout Skeleton (`WalletProfileLayout`):
        *   Top section: Placeholders for "General Account Info" (left) and "Account Summary Snapshot" (right).
        *   Bottom section: Tab container (`WalletTabsContainer` with shadcn/ui `Tabs`) with stubs for the 4-5 data panels.
        *   Use Tailwind grid/flex for responsive layout.
    10. Component Stubs: `AccountSummaryCard`, `TimeRangeSelector`.

**Week 2: Time Filter, Sticky Summary & "Token Performance" Tab**
*   **Tasks:**
    1.  `TimeRangeSelector` Implementation:
        *   Build dropdown/date picker controls using shadcn/ui.
        *   Store selected range globally using Zustand/Context.
        *   Ensure API calls will use this global time range.
    2.  Connect Sticky Summary Bar:
        *   Fetch data from `GET /wallets/{walletAddress}/summary` using SWR/React Query.
        *   Populate "General Account Info" and "Account Summary Snapshot" using Tremor components.
    3.  "Token Performance" Tab Implementation:
    ...
    4.  Integrate basic loading states and error handling for API calls.
