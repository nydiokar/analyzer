...
02 P0 - simple swap is double/triple-spending - assigning the whole value to both (Split) amounts in the transaction
NavpvmB5tdBmzRL1ndyfZtoQ4rfmRY3ZS8sYQJLmjDfk3pT5gckBQrUJV8bMiK1NzAQdSVi5MGRVgwa822t9i7S - out

03 P0 - SOLution we need to reconsider UNKNOWN interactionType from AnalysisResult calculations and all analysis - these are mostly burn or remove/add liquidity and added up several times as above but are adding tons SOL value for the respective token while it's not actual trade = sell or buy  - 4PGRF5ypXR55EUNEwUoDJXWj1GnnxJsCLuy3QnnpT8p8uKGKERnUPqVsNMgv8FsDq6ewpzgcfGG3eJ85SR5SEMaY; One problem here, there are legit unknown but with rather small value, should we make them scapegoat? INIT_PERMISSIONLESS_CONSTANT_PRODUCT_POOL_WITH_CONFIG interactionType is add liquidity, also exclude ... 

### High Priority (Next Working Session Focus)


0. Check the token-performance backend in regard to the recent calculations and analysis introduced. Should we move anything to more appropriate place, is it fine there, do we need to engage and store some stuff into DB, what are the potential overcomplications? 

0.1 Check Dexscreener service, it's now loading all tokens every time when refresh is done, no timestamp, maybe we do it in a smarter way - 5 minutes is fine overall but maybe the refresh on token performance tab to not be trigger it, probably the whole trigger-analysis, or specific button for this for the moment. 

1.  **Implement AI Expert Analysis Interpreter (Phase 6, Use Case 2: Similarity Analysis)** - check behavioral_reconstruction_task.md as blueprint on how to be done
...
1. **AI Insights for Overview Tab (Conceptual Planning & Prototyping):**
