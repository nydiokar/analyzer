...
2.  **Implement Basic Sidebar Wallet Navigation (Search & Favorite Wallets):** - **PARTIALLY COMPLETE / IN PROGRESS**
    ...
    *   **Original Backend Task (Not Yet Implemented / Deprioritized in favor of Favorites):**
        *   Implement `GET /api/v1/wallets/recent?limit=<N>`:
            *   Returns a list of the `N` most recently analyzed or accessed wallets. This could be based on `Wallet.updatedAt`, `Wallet.lastSeenAt` (if implemented), or a combination.
            *   For each wallet, include its address and a small set of key overview stats (approx. 6-7 metrics like PNL, win rate, last activity, top 1-3 tokens, behavior tag – sourced from its latest `/summary` data or equivalent).
    *   **Original Frontend Task (Partially Addressed by Favorites Display):**
        *   Below the search (or in a distinct section), display a list of "Recent Wallets" (e.g., 5-10 wallets) fetched from `/api/v1/wallets/recent`. (Replaced by Favorite Wallets List)
            *   Each item should display the wallet address and its key overview stats. (Favorites list does this)
            *   Clicking an item navigates to `/wallets/<walletAddress>`. (Favorites list does this)
