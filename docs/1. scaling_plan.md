...
## Immediate Focus: Wallet State Enhancement (SOL & Token Balances)

**Goal:** Integrate the fetching of current SOL and all associated SPL Token balances for wallets to enrich analysis and provide up-to-date information for the dashboard and CLI scripts. This involves using efficient batch RPC calls.
...
2.  **Implement Balance Fetching Logic in the chosen Service/Client:**
    ...
    *   **Token Balances:**
        *   For each `walletAddress` in the input array:
            ...
            *   **Crucially, use `encoding: "jsonParsed"`** and provide the `programId` for the SPL Token Program (`TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA`) --- VERIFY THIS!
...
5.  **API & Dashboard Enhancements (Considered for a near-future iteration, post-backend implementation):**
    ...
    *   **Dashboard Integration:**
        *   The "General Account Info" / "Sticky Summary Bar" can display the SOL balance (either from snapshot or live).
        *   A new section or part of the "Token Performance" or "Account Stats" tab could display current token holdings, clearly indicating if they are live or from the last analysis snapshot.
...
## Phase 4: API Layer Implementation (Largely Complete - pending Notes endpoint)
...
8.  **Phase 5: Dashboard Integration (Current Focus)**
