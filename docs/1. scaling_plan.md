...
| ----------- | --------------------------------- | -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| C1          | **Job Status API Endpoints**      | None           | "Create `GET /api/v1/jobs/{jobId}/status`, `GET /progress`, `GET /result` endpoints. Use BullMQ's job status APIs. Return JSON: `{status, progress, result}`.
| C2          | **Job Submission Endpoints**      | C1             | "Add `POST /api/v1/jobs/wallets/sync`, `analyze`, `similarity/analyze` endpoints. Validate input, enqueue job, return job ID and initial status."                    |
| C3          | **Backwards Compatibility Layer** | C2             | "Wrap legacy similarity endpoints to internally create jobs + poll for result. Preserve synchronous interface for old clients. No change to frontend yet."           |
