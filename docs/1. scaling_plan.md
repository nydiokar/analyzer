...
4.  **CLI Script for User/API Key Management (Utility):**
    *   Develop a simple CLI script (e.g., `src/scripts/manage-users.ts`) that uses the `DatabaseService.createUser()` method to allow an administrator to generate new users and API keys from the command line. This is crucial for bootstrapping API access before a full admin UI exists. - **COMPLETE**

## Immediate Focus (Dashboard Development - Phase 5 Kick-off)

1.  **Dashboard Project Setup & Core Dependencies Integration:** - COMPLETE
    *   Initialize the Next.js (App Router) frontend project. - COMPLETE
    *   Integrate essential styling and component libraries: TailwindCSS, shadcn/ui, and Tremor. - COMPLETE
    *   Set up the charting library (Apache ECharts), data fetching library (SWR or React Query), and global state management (Zustand or React Context). - COMPLETE
    *   (Corresponds to Phase 5.3, Week 1, tasks 1-6) - COMPLETE

2.  **Implement Dashboard Layout Skeleton & Basic Routing:** - COMPLETE
    *   Define core application routing (e.g., `/wallets/[walletAddress]`, `/settings`). - COMPLETE
    *   Build the main `WalletProfileLayout` component, including the Sidebar, Top Sticky Summary Bar area, and the container for tabbed data views. - COMPLETE
    *   Create initial component stubs for `AccountSummaryCard` and `TimeRangeSelector`. - COMPLETE
    *   (Corresponds to Phase 5.3, Week 1, tasks 7-10) - COMPLETE

3.  **Develop Global Time Filter & Connect Sticky Summary Bar:**
    *   Implement the `TimeRangeSelector` component, ensuring its selected range is managed via global state (Zustand/Context).
    *   Connect the "Sticky Summary Bar" (within `WalletProfileLayout`) to the `GET /wallets/{walletAddress}/summary` API endpoint, populating it with live data. - COMPLETE
    *   (Corresponds to Phase 5.3, Week 2, tasks 1-2)

## Current Dashboard Development Focus (Extending Phase 5)

**COMPLETED - Functionality for Time-Filtering Implemented**
1.  **Implement Time-Filtering for "Token Performance" Tab:** - **COMPLETE**
    *   **Goal:** Allow users to see token performance (PNL per token, trade counts, etc.) specifically for the selected time range in the dashboard.
    *   **Backend Tasks:**
        *   Update `TokenPerformanceQueryDto` to accept optional `startDate` and `endDate`. - **COMPLETE**
        *   Modify `TokenPerformanceService` to perform aggregation/calculation of token-level PNL and stats from `SwapAnalysisInput` records filtered by the provided time range. This service will need its own analysis logic, potentially reusing parts of `SwapAnalyzer` or `PnlAnalysisService` concepts at a token-specific level. - **COMPLETE**
        *   Update the `GET /wallets/{walletAddress}/token-performance` endpoint in `WalletsController` to use the enhanced service and DTO. - **COMPLETE**
    *   **Frontend Tasks:**
        *   Connect the "Token Performance" tab in `WalletProfileLayout` to the updated API endpoint. - **COMPLETE**
        *   Ensure the SWR hook for this tab passes the global time filter state (`startDate`, `endDate`). - **COMPLETE**
        *   Display the period-specific token performance data (e.g., in a sortable, paginated table). - **COMPLETE**

--- 

## Immediate Tasks for Next Session

1.  **Enhance "Account Stats & PNL" Tab (PNL Overview) for Time-Filtering & Dual Display:**
    *   **Goal:** Provide users with both a time-filtered view of PNL overview stats and an all-time overview in the same tab.
    *   **Backend Tasks:**
        *   Modify `PnlOverviewService` and the `GET /wallets/{walletAddress}/pnl-overview` endpoint.
        *   The endpoint should accept optional `startDate`/`endDate`.
        *   If dates are provided, the service calculates PNL overview for that period (using `PnlAnalysisService.analyzeWalletPnl` with `isViewOnly: true` and the specific time range).
        *   The service should *always* also provide the all-time PNL overview (e.g., by calling `PnlAnalysisService.analyzeWalletPnl` without a time range, or fetching the latest persisted `WalletPnlSummary`).
        *   Update the `PnlOverviewResponse` DTO to structure the response clearly, e.g., with `periodData: PnlOverviewResponseData | null` and `allTimeData: PnlOverviewResponseData`.
    *   **Frontend Tasks:**
        *   Update the "Account Stats & PNL" tab to consume the enhanced API endpoint.
        *   Design the UI to clearly present both the time-filtered overview and the all-time overview (e.g., using separate cards or sections).
...
1.  **Connect "Behavioral Patterns" Tab with Time-Filtering:**
    ...
    *   **Backend Tasks:**
        *   Update the `GET /wallets/{walletAddress}/behavior-analysis` endpoint in `WalletsController` to accept optional `startDate`/`endDate` query parameters (e.g., via a new DTO or by enhancing `BehaviorAnalysisConfig` if appropriate for query params). - **LARGELY COMPLETE - Backend accepts time parameters. Verify data returned is correctly scoped.**
        *   Ensure the controller passes these dates to `BehaviorService.getWalletBehavior`. - **LARGELY COMPLETE**
        *   (The core `BehaviorService` and `OriginalBehaviorService` are already capable of using the time range for data fetching and skipping DB upserts, as recently implemented). - **VERIFIED**
    *   **Frontend Tasks:**
        ...
        *   Display the time-scoped behavioral metrics and visualizations (e.g., heatmaps, classifications relevant to the period). - **VERIFY DISPLAY & VISUALIZATIONS**
...
-   **Styling:** TailwindCSS
-   **Core Component Library:** shadcn/ui (for primitives like tabs, buttons, dropdowns, modals, layout)
...
-   **Main Window - Bottom Section (Tabbed Data Views):**
    ...
    *   **Tab 1: Token Performance** (Formerly "Aggregated SPL Data" / "Aggregated Data")
        *   Purpose: Per-token level performance aggregation.
        *   Content: Sortable, paginated table or card list. Columns/fields: Token, P&L per token, trade count, average hold time, win rate per token, total volume, net amount change.
        *   API: `GET /wallets/{walletAddress}/token-performance` (with query params for pagination, sorting, time range).
...
**Week 2: Time Filter, Sticky Summary & "Token Performance" Tab** - IN PROGRESS
*   **Tasks:**
    1.  `TimeRangeSelector` Implementation:
        ...
        *   Ensure API calls will use this global time range. - IN PROGRESS (Mechanism in place, to be verified with more components)
    2.  Connect Sticky Summary Bar:
        *   Fetch data from `GET /wallets/{walletAddress}/summary` using SWR/React Query. - COMPLETE (with mock API)
        *   Populate "General Account Info" and "Account Summary Snapshot" using Tremor components. - COMPLETE
    3.  "Token Performance" Tab Implementation: - TO DO
        ...
        *   Display data in a shadcn/ui `Table` with pagination and sorting.
        *   Include columns like Token, P&L, Trade Count, Avg Hold Time.
    4.  Integrate basic loading states and error handling for API calls. - IN PROGRESS (Implemented for AccountSummaryCard, ongoing for other components)

**Week 3: "Account Stats & PNL" and "Behavioral Patterns" Tabs** - TO DO
*   **Tasks:**
    1.  "Account Stats & PNL" Tab Implementation:
        *   Fetch data from `GET /wallets/{walletAddress}/pnl-overview` and `GET /wallets/{walletAddress}/summary`.
        *   Use Tremor `Metric`, `Card`, `Grid`, `Flex` to render key stats (Overall PNL, Winrate, Avg PNL/Day, Profit Consistency Index).
        *   Incorporate ECharts for simple distributions if valuable (e.g., PNL distribution).
    2.  "Behavioral Patterns" Tab Implementation:
        *   Fetch data from `GET /wallets/{walletAddress}/behavior-analysis`.
        *   Display trader classification, strategic tags using Tremor `Text`, `Badge`.
        *   Implement ECharts for:
    ...
    3.  Ensure all tabs correctly respond to global `TimeRangeSelector` changes, re-fetching data as needed.

**Week 4: "Reviewer Log/Notes" Tab, (Optional "Raw Transactions" Tab), Polish & Export**
*   **Tasks:**
