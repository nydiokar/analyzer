...
| ----------- | ---------------------------------- | -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- | ---------------------------------------------------------- |
| D1          | **WebSocket Progress Gateway**     | C1             | "Build `JobProgressGateway` using Redis pub/sub. Subscribe to \`job:\${jobId}\:progress                                                                        | completed | failed\`. Emit updates to frontend client over WebSocket." |
| D2          | **Frontend WebSocket Integration** | D1             | "Update frontend to subscribe to job WebSocket channel using returned `jobId`. Show progress bar, real-time updates. Fallback to polling on failure."          |           |                                                            |
| D3          | **Dead Letter Queue Monitoring**   | None           | "Create BullMQ `failed-jobs` queue. On failure, emit metrics and send alert via `AlertingService`. Integrate with existing metrics system (e.g., Prometheus).
| D4          | **Queue Health Endpoint**          | None           | "Create `GET /health/queues` endpoint returning per-queue health using BullMQ stats. Include Redis status and overall health status (healthy/degraded)."       |           |                                                            |
