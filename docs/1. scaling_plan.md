...
### 5.5.1 Auto-Loading Dashboard Wallet Analyses — Execution Plan

**Objective:** deliver an end-to-end experience where opening a wallet profile immediately queues a fast, resource-aware analysis, surfaces fresh KPIs without manual clicks, and—when appropriate—follows up with a deeper historical run. The flow must preserve existing queue behaviour, obey throttles/locks, and keep administrators informed.

#### A. Functional Requirements
- **Trigger policy**
  - Auto-trigger only if the wallet summary is missing, `status === 'unanalyzed'`, or `lastAnalyzedAt` is older than a configurable freshness window (default 6 hours).
  - Respect demo restrictions (no auto-trigger when `useApiKeyStore.isDemo` or API key missing).
  - Deduplicate per wallet per browser session using `sessionStorage` (key: `autoTrigger:<wallet>` storing ISO timestamp) plus an in-memory ref to stop multiple effects from the same render.
- **Scope & payload**
  - Introduce request fields `analysisScope: 'recent' | 'full'`, `historyWindowDays?: number`, `triggerSource: 'auto' | 'manual'`.
  - Auto-trigger uses `{ analysisScope: 'recent', historyWindowDays: 7, triggerSource: 'auto', followUpFullScan: true }`.
  - Manual “Refresh” button defaults to `{ analysisScope: 'full', forceRefresh: true, triggerSource: 'manual' }`.
- **User feedback**
  - Replace idle CTA with “Queued recent analysis…” indicator while the auto job runs.
  - When the recent run completes, toast “Recent window ready” and, if a follow-up job is queued, show “Continuing deep history…” status until it finishes.
  - Maintain existing error handling toasts; add specific copy for 503 “analysis already running”.

#### B. Backend Changes
1. **DTO & job typing**
   - `src/api/shared/dto/dashboard-analysis.dto.ts`: add `analysisScope`, `historyWindowDays`, `triggerSource`. Validate `historyWindowDays` ∈ [1, 30] when scope is `recent`; default scope to `'full'`.
   - `src/queues/jobs/types/index.ts`: extend `DashboardWalletAnalysisJobData` with the new fields plus `followUpFullScan?: boolean`.
2. **Controller logic (`AnalysesController.queueDashboardWalletAnalysis`)**
   - Short-circuit if a `recent` auto-trigger is requested but `DatabaseService.getWalletsStatus` reports `FRESH` within freshness window; return 202 with synthetic `skipped:true` in response body.
   - When scope is `recent`:
     - Force `enrichMetadata = false`.
     - Compute `historyWindowDays` default (7) and include in job data.
     - Downgrade queue priority to `JobPriority.HIGH` instead of `CRITICAL`.
   - Persist `triggerSource` in logs and metric counters.
3. **Processor adjustments (`AnalysisOperationsProcessor.processDashboardWalletAnalysis`)**
   - Compute sync options based on scope:
     - `recent`: `maxSignatures = min(ANALYSIS_EXECUTION_CONFIG.DASHBOARD_MAX_SIGNATURES, 350)`, `fetchOlder = false`, `smartFetch = true`.
     - `full`: keep existing behaviour.
   - If `analysisScope === 'recent'` and PnL data newer than the freshness window already exists, skip heavy work: update progress to 100, return cached results, mark `result.cached = true`.
   - Skip enrichment queueing for `recent` jobs. For `full`, retain enrichment.
   - Append `{ analysisScope, historyWindowDays, triggerSource, followUpFullScanQueued }` to the result published via `JobProgressGateway`.
   - After a successful `recent` job, enqueue a low-priority `full` job when `followUpFullScan === true` **and** no completed `full` job exists in the last 24h (check via `JobsService` or a helper on `DatabaseService`).
4. **Job service (`src/api/services/jobs.service.ts`)**
   - Ensure any direct submissions (if used) forward `analysisScope` and friends.
5. **Metrics & logging**
   - Add structured logs for every auto trigger (wallet, source IP, scope, decision).
   - Emit counters `dashboard_auto_triggers_total{scope=...}` and `dashboard_auto_trigger_skips_total{reason=...}` if Prometheus is wired; otherwise log for manual inspection.

#### C. Frontend Implementation
1. **Type updates**
   - `dashboard/src/types/api.ts`: mirror DTO changes; ensure websocket types (`JobCompletionData`) can carry `analysisScope` and `followUpFullScanQueued`.
2. **Auto-trigger effect (`WalletProfileLayout`)**
   - After SWR delivers `walletSummary`, run `evaluateNeedForAutoTrigger(walletSummary)`:
     - Compare `lastAnalyzedAt` with freshness window.
     - Check session dedupe + demo state.
   - When conditions pass, invoke `handleTriggerAnalysis` with new payload.
   - Track `activeRecentJobId`, `activeFullJobId`, `recentCompletionTimestamp`.
3. **Manual trigger UX**
   - Disable manual button while `recent` job inflight; offer “Run full history now” action if the user insists (confirm modal).
   - If a manual `full` job completes while a background `full` is queued, cancel duplicate follow-up using `jobs/cancel`.
4. **Status components**
   - `AccountSummaryCard`: render labels “Recent snapshot (<7d)” vs “Full history loaded”.
   - Header badge: color-code based on `analysisScope` of the last successful job.
   - Toast manager: differentiate auto vs manual to reduce noise.
5. **Caching tweaks**
   - Temporarily drop `SWR` `dedupingInterval` to 60s while an auto-trigger job is outstanding to ensure the summary refreshes once the job finishes.
   - Invalidate token performance cache only after the `full` job completes (since recent run skips enrichment).

#### D. Operational Guardrails & Edge Cases
- **Concurrency:** rely on existing Redis lock; surface 503 responses plainly so the client can back off.
- **Network failures:** auto-trigger should set a retry timer (e.g., 3m) if the POST fails due to network, but never spin indefinitely.
- **Stale data:** if wallet has < N transactions in last 7 days, still allow recent run; follow-up full ensures totals catch up.
- **Demo mode / missing API key:** bail out early with log `auto-trigger-skipped: demo`.
- **Multiple tabs / users:** dedupe on client + server ensures at most one job. Operators opening the same wallet simultaneously will see the same progress indicators via websocket.

#### E. Verification Plan
- **Unit:** DTO validation tests; processor tests verifying sync options per scope; front-end util tests for `shouldAutoTrigger`.
- **Integration:** end-to-end API test simulating recent scope request, asserting job data and websocket payload; ensure follow-up full job is queued once.
- **Manual checklist:** run through fresh wallet, stale wallet, demo user, intermittent websocket, manual override scenarios and capture timings.

#### F. Follow-up Tasks (Optional but Valuable)
- Persist auto-trigger audit entries in `ActivityLog` table for compliance.
- Expose last `analysisScope` + timestamp in admin dashboard for monitoring.
- Make freshness window & history days configurable via `.env` (`DASHBOARD_RECENT_HISTORY_DAYS`, `DASHBOARD_ANALYSIS_FRESHNESS_MINUTES`).
- Consider rate-limiting auto triggers per user/IP at API gateway if abuse detected.
