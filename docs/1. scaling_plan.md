...
| C4          | **Frontend Job Trigger Hook**     | C2             | | "Implement frontend call to `POST /api/v1/jobs/similarity/analyze`. Store returned job ID, subscribe via polling or WebSocket. Replace old similarity flow trigger." |
| C5          | **Frontend Job Result Display**   | C1, C4         |  | "Update similarity results page to poll job result via `/status` + `/result` endpoints. Display loading/progress bar. Fallback to sync if job failed."               |
