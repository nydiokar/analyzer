...
2.  **Implement Basic Sidebar Wallet Navigation (Search & Favorite Wallets):** - **COMPLETE**
    ...
    *   **Status & Summary:**
        ...
        *   **"Recent List" evolved into "Favorite Wallets":**
            ...
            *   **Frontend:**
                ...
                *   Displays favorite wallets with address, PNL, and score (if available). Allows removal, copying address. - **COMPLETE**
...
3.  **Define and Implement Consistent Loading, Empty, and Error States Across All Tabs/Components:** - **IN PROGRESS (Documentation & Final Review Next)**
    ...
    *   **Next Steps:**
        *   **Documentation:** Create a dedicated subsection (e.g., "5.2.1 Consistent UI States" or a new one if previously removed) within this plan. This section will detail:
            *   Design principles for each state.
            *   Visual guidelines (e.g., when to use specific `EmptyState.tsx` variants, iconography, text styling).
            *   Specific scenarios and recommended user messages for:
                *   Loading States: Full page/tab loading, partial/component-level loading, use of skeleton loaders.
                *   Empty States: No data for selected criteria, new wallet without analysis, no transactions found, no search results, etc. Clear calls to action (CTAs) should be defined (e.g., "Analyze This Wallet", "Adjust Filters").
                *   Error States: API errors (network issues, server errors), client-side processing errors. Messages should be user-friendly, with clear retry options or guidance.
            *   Recommended components (e.g., specific shadcn/ui or Tremor components for alerts, toasts, skeleton placeholders).
        *   **Comprehensive Review & Implementation Pass:**
            *   Systematically review all existing dashboard tabs, components, and pages (including the main landing page `dashboard/src/app/page.tsx` and `ReviewerLogNotesTab.tsx`) to ensure they fully adhere to the documented guidelines.
            *   Implement or update loading, empty, and error states where missing or inconsistent.
            *   Pay special attention to ensuring consistency in visual appearance, messaging, and behavior of CTAs.
        *   **Testing Strategy:**
            *   For each relevant component/tab:
                *   Verify correct display and behavior of loading states (consider simulating network latency).
                *   Verify correct display and behavior of empty states (e.g., by using a new wallet with no data, or applying filters that result in no data).
                *   Verify correct display and behavior of error states (e.g., by simulating API errors or invalid data responses).
            *   Confirm that all CTAs (e.g., "Analyze Wallet", "Retry Analysis") function as expected.
            *   Ensure a consistent user experience across the entire dashboard regarding these states.
...
1.  **`EmptyState.tsx` Component:**
    *   **Variants:** Utilize the predefined variants (`default`, `error`, `info`, `playful`, `loading`) appropriately.
        *   `loading`: For general loading states where a specific skeleton loader is not used.
        *   `error`: For API errors or critical client-side errors.
        *   `info`: For informational messages, like "No data available."
        *   `default`/`playful`: Can be used for empty states where a more engaging or less formal message is appropriate (e.g., a brand new user with no wallets).
    *   **Props:**
        *   `icon`: Use appropriate icons (e.g., `Loader2` for loading, `AlertTriangle` for error, `Info` for info, `SearchX` for no results).
        *   `title`: Clear, concise title for the state.
        *   `description`: Further explanation or guidance.
        *   `actionButton`: Include if a direct call to action is relevant (e.g., "Retry", "Analyze Wallet").

2.  **Skeleton Loaders (shadcn/ui `Skeleton`):**
    *   Use for component-level loading states where the structure of the content is known. This provides a better UX than a generic loading spinner by showing the shape of the content to come.
    *   Apply to cards, tables, charts, and text blocks during their initial data fetch.

3.  **Toasts (shadcn/ui `useToast`):**
    *   For non-modal notifications like success messages (e.g., "Wallet added to favorites"), warnings, or less critical errors that don't require a full `EmptyState` display.
    *   Use `variant: 'destructive'` for error toasts.

4.  **Inline Indicators:**
    *   For very localized loading (e.g., a single metric updating), a small spinner (`Loader2` icon with `animate-spin`) next to the metric can be less intrusive.

5.  **Button States (shadcn/ui `Button`):**
    *   Utilize the `disabled` and loading states for buttons that trigger asynchronous actions (e.g., show a spinner within the button and disable it during an API call).

**C. Specific Scenarios & Recommended Messages/Actions:**

1.  **Loading States:**
    *   **Full Page/Tab Loading:**
        *   **Visual:** `EmptyState` component with `loading` variant or multiple `Skeleton` components matching the page layout.
        *   **Title:** "Loading [Page/Tab Name]..." (e.g., "Loading Token Performance...")
        *   **Description:** (Optional) "Please wait while we fetch the latest data."
    *   **Component-Level Loading (e.g., a single chart or card):**
        *   **Visual:** `Skeleton` component matching the component's structure. If not feasible, a compact `EmptyState` with `loading` variant or an inline spinner.
        *   **Title (if `EmptyState` used):** "Loading..."
    *   **Background Data Refresh (SWR revalidation):**
        *   Generally, no explicit global loading state is needed if data is already displayed. Consider subtle indicators if the refresh is user-initiated and takes noticeable time.

2.  **Empty States:**
    *   **No Data for Selected Criteria (e.g., filters, time range):**
        *   **Visual:** `EmptyState` with `info` variant.
        *   **Icon:** `SearchX` or `Info`.
        *   **Title:** "No Data Found" or "No [Items] Match Your Criteria" (e.g., "No Transactions Match Your Filters").
        *   **Description:** "Try adjusting your filters or expanding the time range."
        *   **Action:** (Optional) Button to "Clear Filters" or "Reset View."
    *   **New Wallet (No Analysis Yet):**
        *   **Visual:** `EmptyState` with `info` variant.
        *   **Icon:** `Info` or a relevant icon like `Activity`.
        *   **Title:** "[Wallet Address] Has Not Been Analyzed Yet"
        *   **Description:** "Analyze this wallet to see its performance, behavior, and statistics."
        *   **Action:** `Button` "Analyze This Wallet" (triggers analysis API).
    *   **No Transactions Found (after analysis, for a specific period):**
        *   **Visual:** `EmptyState` with `info` variant.
        *   **Icon:** `Info`.
        *   **Title:** "No Transactions Recorded"
        *   **Description:** "There were no transactions for this wallet within the selected period."
    *   **No Search Results (e.g., wallet search):**
        *   **Component:** `WalletSearch.tsx` already handles this.
        *   **Message:** "No wallets found. You can import and analyze a new wallet."
        *   **Action:** "Import & Analyze" button.
    *   **Empty Favorite Wallets List:**
        *   **Component:** `FavoriteWalletsList.tsx` already handles this.
        *   **Message:** "No favorite wallets yet. Star a wallet to add it here."

3.  **Error States:**
    *   **API Errors (Failed to fetch data for a tab/component):**
        *   **Visual:** `EmptyState` with `error` variant.
        *   **Icon:** `AlertTriangle` or `WifiOff`.
        *   **Title:** "Error Loading Data" or "Could Not Fetch [Data Type]" (e.g., "Could Not Fetch PNL Overview").
        *   **Description:** "An error occurred while fetching data. Please check your connection or try again. If the problem persists, contact support." (Optionally, show a generic error message and log the detailed error to the console).
        *   **Action:** `Button` "Retry" (re-fetches data).
    *   **Client-Side Processing Errors (Unexpected error in component):**
        *   **Visual:** `EmptyState` with `error` variant or a React Error Boundary.
        *   **Icon:** `AlertTriangle`.
        *   **Title:** "An Unexpected Error Occurred"
        *   **Description:** "Something went wrong while displaying this section. Please try refreshing the page."
    *   **API Key Not Configured / Invalid (for features requiring it, e.g., Helius):**
        *   **Visual:** `EmptyState` with `error` or `info` variant.
        *   **Icon:** `KeyRound` or `Settings`.
        *   **Title:** "API Key Configuration Needed" or "Invalid API Key"
        *   **Description:** "This feature requires a valid Helius API key. Please configure it in settings."
        *   **Action:** (Optional) `Button` "Go to Settings".
    *   **Specific Action Failure (e.g., failing to add a favorite):**
        *   **Visual:** Toast notification with `variant: 'destructive'`.
        *   **Title:** "Action Failed" (e.g., "Failed to Add Favorite").
        *   **Description:** (Optional) Brief reason if available.
