...
#### Auth gaps and decisions (2025-09)

- What’s implemented now
  - Composite JWT + `X-API-Key` guard; endpoint throttling; demo-user gating.
  - Email verification model; optional cookie mode with `HttpOnly; SameSite=strict`.
  - Password hashing with bcrypt (12 rounds) + pepper; timing attack padding.

- Important gaps to address for a safe release
  - Session model: short-lived access token (≈15–30m) plus rotating refresh tokens stored in Redis with per-session IDs, server-side revocation on logout/password reset.
  - CSRF for cookie mode: if `AUTH_COOKIE_MODE=true`, add double-submit or signed per-mutation CSRF token middleware and frontend header.
  - Key management: add `kid` and rotation policy now; plan migration to asymmetric signing (RS256/EdDSA) with JWKS endpoint.
  - Recovery flows: password reset tokens (hashed at rest, single-use), session invalidation after reset; email-change verification with anti-replay.
  - JWT hardening: validate `iss`/`aud`/`nbf`, add ±60–120s clock skew tolerance.
  - API keys: prefixes, scope-limited keys, per-key quotas/rotation UX (keep current hashing-at-rest).

- Argon2id vs bcrypt (decision)
  - Current: bcrypt (12) + pepper is acceptable for v1 release. Argon2id offers better memory-hard resistance.
  - Migration approach: introduce `passwordHashAlgo` (or version prefix) and dual-verify → rehash on successful login. Defer until after session/CSRF/key-rotation are in place.

- Release baseline (v1)
  - Access token TTL 15–30m; rotating refresh tokens in Redis; per-session logout and revocation.
  - CSRF protection enabled when cookie mode is on.
  - `kid` added with documented rotation; plan to move to RS256/EdDSA later.
  - Password reset flow with session invalidation.

- Deferred (v2+ nice-to-haves)
  - Argon2id migration; device binding for refresh tokens; IP reputation; RBAC/ABAC policy layer; OAuth/OIDC.
