...
| **Task ID** | **Task**                        | **Depends On** | **Status** | **Instructions to LLM**                                                                                                                                          |
| ----------- | ------------------------------- | -------------- | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| B1          | **WalletOperationsProcessor**   | None           | ✅ **COMPLETE** | "Implement `WalletOperationsProcessor` to sync wallets using `HeliusSyncService`. Add Redis lock-based idempotency. Use progress tracking."                      |
| B2          | **AnalysisOperationsProcessor** | None           | ✅ **COMPLETE** | "Implement `AnalysisOperationsProcessor` with two job handlers: PNL and behavior. Wrap existing `PnlAnalysisService` and `BehaviorService`. Track job progress." |
| B3          | **Job Status API**              | None           | ✅ **COMPLETE** | "Build `JobStatusController` with endpoints to fetch job status, progress, and result. Use BullMQ API."                                                          |
| B4          | **Similarity Job Trigger API**  | B3             | ✅ **COMPLETE** | "Create `POST /api/v1/jobs/similarity/analyze` endpoint that adds a similarity-flow job using BullMQ. Return job ID and initial status."                         |
| B5          | **WebSocket Progress Gateway**  | B3             | ✅ **COMPLETE** | "Implement `JobProgressGateway` using Redis pub/sub to forward job progress, completion, and failure events over WebSocket. Use BullMQ `QueueEvents`."  
...
| **Task ID** | **Task**                          | **Depends On** | **Status** | **Instructions to LLM**                                                                                                                                              |
| ----------- | --------------------------------- | -------------- | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| C1          | **Job Status API Endpoints**      | None           | ✅ **COMPLETE** | "Create `GET /api/v1/jobs/{jobId}/status`, `GET /progress`, `GET /result` endpoints. Use BullMQ's job status APIs. Return JSON: `{status, progress, result}`."
| C2          | **Job Submission Endpoints**      | C1             | ✅ **COMPLETE** | "Add `POST /api/v1/jobs/wallets/sync`, `analyze`, `similarity/analyze` endpoints. Validate input, enqueue job, return job ID and initial status."                    |
| C3          | **Backwards Compatibility Layer** | C2             | ✅ **COMPLETE** | "Wrap legacy similarity endpoints to internally create jobs + poll for result. Preserve synchronous interface for old clients. No change to frontend yet."           |
| C4          | **Frontend Job Trigger Hook**     | C2             | ✅ **COMPLETE** | "Implement frontend call to `POST /api/v1/jobs/similarity/analyze`. Store returned job ID, subscribe via polling or WebSocket. Replace old similarity flow trigger." |
| C5          | **Frontend Job Result Display**   | C1, C4         | ✅ **COMPLETE** | "Update similarity results page to poll job result via `/status` + `/result` endpoints. Display loading/progress bar. Fallback to sync if job failed."               |



| **Task ID** | **Task**                           | **Depends On** | **Status** | **Instructions to LLM**                                                                                                                                        |
| ----------- | ---------------------------------- | -------------- | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| D1          | **WebSocket Progress Gateway**     | C1             | ✅ COMPLETE | "Build `JobProgressGateway` using Redis pub/sub. Subscribe to \`job:\${jobId}\:progress\|completed\|failed\`. Emit updates to frontend client over WebSocket." |
| D2          | **Frontend WebSocket Integration** | D1             | ✅ BACKEND READY | "Update frontend to subscribe to job WebSocket channel using returned `jobId`. Show progress bar, real-time updates. Fallback to polling on failure."          |
| D3          | **Dead Letter Queue Monitoring**   | None           | ✅ COMPLETE | "Create BullMQ `failed-jobs` queue. On failure, emit metrics and send alert via `AlertingService`. Integrate with existing metrics system (e.g., Prometheus)." |
| D4          | **Queue Health Endpoint**          | None           | ✅ COMPLETE | "Create `GET /health/queues` endpoint returning per-queue health using BullMQ stats. Include Redis status and overall health status (healthy/degraded)."       |
...
*   **The New Hybrid Strategy:** We will pursue a dual-pronged approach:
