...
1.  **Implement Analysis Triggering API Endpoint & Frontend Integration (Simplified Initial Version):**
    *   **Goal:** Enable users to initiate data fetching and analysis for wallets not yet in the system or requiring an update, directly from the dashboard. This is critical for usability.
    *   **Backend Tasks (`POST /api/v1/wallets/{walletAddress}/analyze`):**
        *   Define and implement the `POST /api/v1/wallets/{walletAddress}/analyze` endpoint.
        *   For this initial version, the endpoint can synchronously execute:
            *   `HeliusSyncService` to fetch/update all transactions for the wallet.
            *   `PnlAnalysisService` to perform PNL and advanced stats calculations.
        *   Upon successful completion, ensure the endpoint updates the `Wallet` model's `analyzedTimestampStart` and `analyzedTimestampEnd` fields.
        *   Return a success response or appropriate error status.
    *   **Frontend Tasks:**
        *   In dashboard tabs (e.g., PNL Overview, Token Performance), if data loading fails due to the wallet not being analyzed or data being stale (heuristics to be defined), display a clear message and an "Analyze This Wallet" (or "Refresh Analysis") button.
        *   On button click, call the new `POST /api/v1/wallets/{walletAddress}/analyze` endpoint.
        *   Provide clear UI feedback during the analysis process (e.g., loading spinner, "Analysis in progress..." message on the button or relevant section).
        *   Upon successful API response, automatically attempt to reload the data for the current tab/view.
...
2.  **Implement Basic Sidebar Wallet Navigation (Search & Recent List):**
    *   **Goal:** Improve wallet discovery and navigation within the dashboard.
    *   **Backend Tasks:**
        *   Implement `GET /api/v1/wallets/search?query=<search_term>`:
            *   Searches the `Wallet` table for addresses matching the `search_term` (e.g., partial match).
            *   Returns a list of matching wallet addresses.
    ...
    *   **Frontend Tasks (Main Application Sidebar):**
        *   Add a search input field to the sidebar.
        *   As the user types, call the `/api/v1/wallets/search` endpoint and display matching wallet addresses as clickable links that navigate to `/wallets/<walletAddress>`.
        *   Below the search (or in a distinct section), display a list of "Recent Wallets" (e.g., 5-10 wallets) fetched from `/api/v1/wallets/recent`.
            *   Each item should display the wallet address and its key overview stats.
            *   Clicking an item navigates to `/wallets/<walletAddress>`.

3.  **Define and Implement Consistent Loading, Empty, and Error States Across All Tabs/Components:**
    ...
    *   **Tasks:**
        ...
        *   **Implementation & Review:**
            *   Review all existing dashboard tabs (`TokenPerformanceTab`, `AccountStatsPnlTab`, `BehavioralPatternsTab`, `ReviewerLogTab`) and any new components.
