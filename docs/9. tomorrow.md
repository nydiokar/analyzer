...
Check the if wallet balances and cache balances could not be the one and what is the difference, we could streamline; 

Create bullmq worker async system (Dont we have it already?); 

Global Token state/registry, why is missing we have tokeninfo table in the DB? 



5 High-Impact Tasks for Next Session
Task 1: Fix Token Enrichment Job Tracking (Critical Bug)
Problem: trigger_token_enrichment jobs show "INITIATED" but no SUCCESS/FAILURE status
Action:
Add proper job ID tracking through BullMQ in enrichment-operations.processor.ts
Implement WebSocket events for job completion/failure
Fix the job status reporting in the enrichment flow
Task 2: Implement BullMQ Worker System for Async Enrichment
Problem: No workers = no asynchronous enrichment, everything happens in request/response cycle
Action:
Set up BullMQ queue: token-enrichment.queue.ts
Create worker: token-enrichment.worker.ts
Implement job lifecycle: enqueue unknown mints → worker resolves → store results
Add retry/backoff logic for failed enrichments
Task 3: Fix Critical UI Structural Inconsistencies
Problem: Several breaking UI bugs identified
Action:
Fix EnhancedKeyInsights expecting results.walletVectorsUsed but server payload not guaranteeing it
Add guards for missing keys
Fix ScoreBar width calculation (lacks clamp, score > 1 renders off-bounds)
Fix capitalOverlapPctA/B computation exceeding 100%
Task 4: Implement Global Token State/Registry
Problem: No global token registry = each request starts from zero, causing high latency
Action:
Create tokens table in database for persistent storage
Implement Redis cache layer: token:price:<mint> and token:status:<mint>
Add LRU/TTL-based eviction for Redis
Create combined approach: DB for persistence + Redis for fast access
Task 5: Debug Capital Allocation Issues
Problem: Listed as #1 priority, likely affecting similarity calculations
Action:
Investigate capital allocation debugging mentioned in the file
Check if there are issues with weight calculations or vector computations
Verify capital overlap percentages are being computed correctly
Test with known wallet pairs to validate calculations
Impact Priority:
Tasks 1-2: Fix current broken functionality (enrichment jobs)
Task 3: Fix user-facing bugs that break the interface
Tasks 4-5: Performance and accuracy improvements
These tasks address both immediate bugs and foundational improvements that will unblock further
